...
export class PagosComponent implements OnInit {
  ...
  CalcularValorTablas(datos: any){
    ...
    if (fechaInicioContrato.getFullYear() == this.anio && fechaInicioContrato.getMonth() + 1 == this.mes) {
      ...
      // let canonIncremento = Math.round((((((datos.incremento + datos.incremento_adicional) / 100) * datos.valor_canon)
      //  + datos.valor_canon) / 30) * diasTrabajar)
      // console.log(canonIncremento, "canon incremento");       
      ...
      // console.log(this.canonIncremento);
      ...
      {              
        ...
        for (let index = 0; index < contratoIncremento[0].conceptos.length; index++) {
          ...
          if(element[index].id_concepto_concepto.incremento == 1 && element[index].id_concepto_concepto.tipo_concepto != 5){
            ...
            actual.valor_incremento = parseInt(element[index].valor)      
            listaInc.push(actual)
  ...
  comprobantePdfNoPagados(base64, datos, tipoPago) {
    ...
    if(fechaIn.getFullYear() < this.anio && fechaIn.getMonth()+1 == this.mes && fechaFin.getFullYear() > this.anio){
      ...
      totalDevengado = Math.round(this.valorTotalConceptos(conceptosDevengados, tipoPago)) + this.contratoIncremento
